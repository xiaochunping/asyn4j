...
public class AsynServiceImpl implements AsynService {
	...
	private AsynServiceImpl() {
		this(maxCacheWork, addWorkWaitTime, work_thread_num,
				callback_thread_num, closeServiceWaitTime);
	}
		...
		try {
			if (instance == null) {
				instance = new AsynServiceImpl(maxCacheWork, addWorkWaitTime,
						workThreadNum, callBackThreadNum, closeServiceWaitTime);
			}
		} finally {
			lock.unlock();
		}
	...
	public void init() {
		...
		if (!run) {
			...
			if (workQueueFullHandler != null) {
				workExecutor = new AsynThreadPoolExecutor(work_thread_num,
						work_thread_num, 0L, TimeUnit.MILLISECONDS, workQueue,
						new AsynThreadFactory(),
						new AsynWorkRejectedExecutionHandler(
								workQueueFullHandler), executeWorkNum);
			} else {
				workExecutor = new AsynThreadPoolExecutor(work_thread_num,
						work_thread_num, 0L, TimeUnit.MILLISECONDS, workQueue,
						new AsynThreadFactory(), executeWorkNum);
			}

			// init callback queue
			callBackQueue = new LinkedBlockingQueue<Runnable>();

			callBackExecutor = new CallBackThreadPoolExecutor(
					callback_thread_num, callback_thread_num, 0L,
					TimeUnit.MILLISECONDS, callBackQueue,
	...
	public void close(long waitTime) {
		if (run) {
			...
			try {
				...
				callBackExecutor.awaitTermination(waitTime,
	...
	public boolean addWork(Object tagerObject, String method, Object[] params,
		...
		return addWork(tagerObject, method, params, asynCallBack,
				DEFAULT_WORK_WEIGHT);
	}

	@Override
	public boolean addWork(Object tagerObject, String method, Object[] params,
			AsynCallBack asynCallBack, WorkWeight weight) {
		return addWork(tagerObject, method, params, asynCallBack,
				DEFAULT_WORK_WEIGHT, false);
	}

	@Override
	public boolean addWork(Object tagerObject, String method, Object[] params,
			AsynCallBack asynCallBack, WorkWeight weight, boolean cache) {
		...
		Object target = null;
		if (tagerObject instanceof String) {
			this.addWorkWithSpring((String) target, method, params,
					asynCallBack, weight);
	...
	@Override
	public boolean addWorkWithSpring(String target, String method,
			Object[] params, AsynCallBack asynCallBack, WorkWeight weight) {
	...
	public boolean addAsynWork(AsynWork asynWork) {
		...
		try {
			...
			if (semaphore.tryAcquire(addWorkWaitTime, TimeUnit.MILLISECONDS)) {//
				WorkProcessor workProcessor = new WorkProcessor(asynWork,
						errorAsynWorkHandler, callBackExecutor, semaphore);
				// asyn work execute
	...
	public String getRunStatInfo() {
		if (run) {
			...
			infoSb.append("total asyn work:").append(totalWork.get())
					.append("\t");
			infoSb.append(",excute asyn work:").append(executeWorkNum.get())
					.append("\t");
			infoSb.append(",callback asyn result:").append(callBackNum.get())
					.append("\t");
		}
	...
	@Override
	public void setErrorAsynWorkHandler(
